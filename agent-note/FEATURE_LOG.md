...
- **原生 WebDAV 支持**：新增 `webdav.js` 后台服务，支持通过原生 `fetch` 使用 Basic Auth 直接连接类似坚果云、Nextcloud 等标准 WebDAV 网盘。
- **全量设置备份**：修改了备份的核心逻辑（`backup.js`），将包含首页封面（`homeHeroImageDataUrl`）在内的全部系统 `settings` 首次加入导出清单。从此换电脑也能 100% 还原个性化封面与配置。
- **自动化同步机制**：在“设置”页新增了“云端同步”配置模块。用户可配置 WebDAV 账密，并开启“每次启动时自动静默上传”，在 `main.js` 初始化阶段自动静默推送最新学习进度。
- **版本交付**：完成开发，合并提交并触发 `v0.1.11` 的云端打包。
